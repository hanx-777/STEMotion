...
| --- | --- | --- | --- | --- |
| 作品完成度 | 是否形成可运行、可演示、可验证的完整作品 | `/rag` 参赛展示页、RAG API、学科 Skill、知识来源、典型案例、斜抛可视化、`/settings` 模型配置 | `src/app/rag`、`src/components/rag`、`src/app/api/rag`、`skills/physics_mechanics`、`src/app/settings` | 补充部署地址、演示视频、更多真实课程知识库 |
| 创意实用度 | 是否贴合真实教学场景，是否解决具体痛点 | 学生分步学习、错因诊断、教师备课、引用追溯、运动轨迹可视化 | `/rag` 任务模式、`rag_pipeline`、`visualization_hint`、典型案例 | 引入真实课堂任务、教师试用反馈和课程作业场景 |
