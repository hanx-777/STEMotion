...
学科智引：基于RAG的垂类大模型助学助教平台 是面向高校课程教学场景的智能助学与助教系统。当前版本把学科 Skill、课程知识库、本地检索、可选网络补充、结构化 RAG 回答、教师备课、互动可视化生成和本地教学资产库串成一个闭环。
...
系统的主导航入口是 `/learn`、`/teach`、`/lab`、`/assets`、`/knowledge` 和 `/settings`。其中 `/learn` 与 `/teach` 复用 RAG 工作台，但模式、默认问题和任务类型不同；`/lab` 与 `/deep-interaction` 复用深度交互工作台；`/assets` 管理本地保存的互动 artifact；`/settings` 管理模型 Profile。`/student`、`/teacher`、`/visualization`、`/interactions` 和 `/rag` 保留为兼容入口。

```mermaid
flowchart LR
  U["用户/评审"] --> Nav["主导航"]
  Nav --> Student["/learn 学生学习"]
  Nav --> Teacher["/teach 教师教学"]
  Nav --> Viz["/lab 可视化实验"]
  Nav --> Library["/assets 教学资产"]
  Nav --> Knowledge["/knowledge 知识库"]
...
| 路径 | 当前行为 | 核心功能 |
| --- | --- | --- |
| `/learn` | 主入口，返回 200 | 学生学习：知识讲解、分步解题、错因诊断 |
| `/teach` | 主入口，返回 200 | 教师教学：课堂备课、演示设计、练习生成 |
| `/lab` | 主入口，返回 200 | 深度交互工作台，生成大学物理力学交互实验 |
| `/assets` | 主入口，返回 200 | 教学资产库，搜索、筛选、打开、删除 artifact |
| `/knowledge` | 主入口，返回 200 | 知识库健康度、源文件和 chunk 覆盖 |
| `/settings` | 主入口，返回 200 | OpenAI 兼容接口与 Claude Messages API Profile 管理 |
| `/student` | 兼容入口 | 重定向到 `/learn` |
| `/teacher` | 兼容入口 | 重定向到 `/teach` |
| `/visualization` | 兼容入口 | 进入或兼容 `/lab` 工作台 |
| `/interactions` | 兼容入口 | 重定向到 `/assets` |
| `/rag` | 兼容入口 | 重定向到 `/learn` |
| `/experiments` | 兼容入口 | 重定向到 `/assets` |
| `/generate` | 兼容入口 | 重定向到 `/visualization` |
| `/player` | 兼容入口 | 重定向到 `/interactions` |
| `/deep-interaction` | 高级入口 | 与 `/lab` 使用同一深度交互工作台 |
...
`/learn` 面向学生学习场景，默认学科是 `physics_mechanics`，显示名为“大学物理力学”。页面顶部提供学科 Skill 选择、Skill 配置查看、网络检索开关、快速模式开关和可视化模式单选。`/student` 作为旧链接兼容入口保留。
