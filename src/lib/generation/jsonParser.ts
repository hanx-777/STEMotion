...
/**
 * Extract LaTeX blocks and replace with placeholders for safer JSON parsing.
 * Handles: $$...$$ (display), $...$ (inline), \[...\] (display), \(...\) (inline)
 */
function extractLatexBlocks(text: string): { cleaned: string; blocks: string[] } {
  const blocks: string[] = [];
  let cleaned = text;
  ...
  cleaned = cleaned.replace(/\$\$([\s\S]*?)\$\$/g, (_match, content) => {
    blocks.push(content);
    return `"__LATEX_${blocks.length - 1}__"`;
  });
...
/**
 * Restore LaTeX blocks from placeholders after parsing.
 */
function restoreLatexBlocks(obj: unknown, blocks: string[]): unknown {
  if (typeof obj === 'string') {
    return obj.replace(/__LATEX_(\d+)__/g, (_match, index) => {
      const blockIndex = parseInt(index, 10);
      return blocks[blockIndex] ?? _match;
    });
  }
  if (Array.isArray(obj)) {
    return obj.map(item => restoreLatexBlocks(item, blocks));
  }
  if (obj && typeof obj === 'object') {
    const result: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(obj)) {
      result[key] = restoreLatexBlocks(value, blocks);
    }
    return result;
  }
  return obj;
...
/**
 * Robust JSON parser with 6 fallback strategies (inspired by OpenMAIC):
 * 0. LaTeX-aware preprocessing (extract math, parse, restore)
 ...
 * 5. Sanitized jsonrepair (nuclear option)
 */
export function parseJsonResponse(raw: string): unknown {
  ...
  // Strategy 0: LaTeX-aware preprocessing
  for (const candidate of candidates) {
    try {
      const { cleaned, blocks } = extractLatexBlocks(candidate);
      const parsed = JSON.parse(cleaned);
      const result = restoreLatexBlocks(parsed, blocks);
      log.debug('parseJson success', { strategy: 'latex_extraction', inputLen: raw.length });
      return result;
    } catch (e) { log.debug('Strategy latex_extraction failed', { error: truncate(String(e instanceof Error ? e.message : e), 200) }); }
  }
  ...
  try {
    ...
    log.error('parseJson exhausted all strategies', {
      inputLen: raw.length,
      preview,
      strategiesAttempted: 6
    });
    throw new Error(`Failed to parse JSON after 6 strategies. Input length: ${raw.length}`);
  }
}
