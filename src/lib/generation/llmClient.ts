...
async function generateWithAnthropic({ baseURL, apiKey, model, timeout, messages, temperature, maxTokens, thinking, stream: wantStream = true }: GenerateParams): Promise<string> {
  ...
  if (useStream) {
    try {
      ...
      if (/terminated|ECONNRESET|ETIMEDOUT|network|fetch failed/i.test(msg)) {
        log.warn('Stream failed, falling back to non-streaming', { error: msg });
  ...
  async function doRequest(stream: boolean): Promise<string> {
    ...
    try {
      ...
      log.info(`Request → ${url}`, { model, maxTokens, provider: 'anthropic', stream, thinking: !!thinking });
      const startTime = Date.now();
      ...
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      log.info(`Response ← 200${stream ? ' (stream)' : ''}`, { chars: content.length, elapsed: `${elapsed}s`, inputTokens, outputTokens, stopReason });
