...
import { judgeEvaluations } from './agents/judgeAgent';
import { repairArtifact } from './agents/repairAgent';
...
export async function runAgentWidgetPipeline(
  ...
  const plan = await planInteraction(input.prompt, preferredType, input.gradeLevel);
  ...
  const html = templateHtml
    ? await ensureWidgetHtmlSafety(plan, templateHtml, blueprint)
    : await buildWidgetHtml(plan, blueprint);
  ...
  const teacher = await buildTeacherActions(plan, widgetConfig.messageTargets, blueprint);
  ...
  let currentActions = teacher.actions;
  ...
  emit({ type: 'feedback_started', message: '开始多 Agent 质量评审...', progress: 89 });

  // Pre-gate: run Safety + Runtime before loop (instant, programmatic)
  // If both pass, skip them in iteration 1 to save time
  const preSafetyEval = evaluateSafety(currentHtml);
  const preRuntimeEval = evaluateRuntime(currentHtml);
  const preGatePassed = preSafetyEval.passed && preRuntimeEval.passed;
  if (preGatePassed) {
    log.info('Pre-gate passed (Safety + Runtime), will skip in iteration 1');
  }

  let bestArtifact = baseArtifact;
  let bestScore = 0;
  let prevScore = 0;
  ...
  for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
    checkAborted();
    const iterProgress = 89 + Math.min(iteration * 3, 9);
    emit({ type: 'feedback_iteration_started', iteration, maxIterations: MAX_ITERATIONS, message: `第 ${iteration}/${MAX_ITERATIONS} 轮评审...`, progress: iterProgress });

    const evaluations: AgentEvaluation[] = [];

    // Pedagogy + UX Evaluators (parallel — no data dependency)
    emit({ type: 'evaluator_started', iteration, agentName: 'Pedagogy Evaluator', message: '教学评估中...', progress: iterProgress });
    emit({ type: 'evaluator_started', iteration, agentName: 'UX Evaluator', message: '交互体验评估中...', progress: iterProgress });
    const [pedEval, uxEval] = await Promise.all([
      evaluatePedagogy({
        ...
        htmlPreview: currentHtml.slice(0, 800),
    ...
    evaluations.push(pedEval, uxEval);
    emit({ type: 'evaluator_completed', iteration, evaluation: pedEval, progress: iterProgress });
    emit({ type: 'evaluator_completed', iteration, evaluation: uxEval, progress: iterProgress });

    // Safety + Runtime Evaluator (programmatic)
    // In iteration 1, reuse pre-gate results if both passed; otherwise re-run
    if (iteration === 1 && preGatePassed) {
      evaluations.push(preSafetyEval, preRuntimeEval);
    } else {
      emit({ type: 'evaluator_started', iteration, agentName: 'Safety Evaluator', message: '安全合规检查中...', progress: iterProgress });
      const safetyEval = evaluateSafety(currentHtml);
      evaluations.push(safetyEval);
      emit({ type: 'evaluator_completed', iteration, evaluation: safetyEval, progress: iterProgress });

      emit({ type: 'evaluator_started', iteration, agentName: 'Runtime Evaluator', message: '运行时检查中...', progress: iterProgress });
      const runtimeEval = evaluateRuntime(currentHtml);
      evaluations.push(runtimeEval);
      emit({ type: 'evaluator_completed', iteration, evaluation: runtimeEval, progress: iterProgress });
    }

    // Judge decision
    const decision = judgeEvaluations(evaluations, iterations[iterations.length - 1]);
    emit({ type: 'judge_decision', iteration, decision, progress: iterProgress });
    log.info('Judge decision', { iteration, type: decision.type, score: decision.finalScore, target: decision.target });
    ...
    if (decision.type === 'accept') {
      log.info('Feedback loop accepted', { iteration, score: decision.finalScore });
      break;
    }

    if (decision.type === 'reject') {
      log.warn('Feedback loop rejected', { iteration, score: decision.finalScore });
      break;
    }

    // Repair
    emit({ type: 'repair_started', iteration, target: decision.target ?? 'all', message: '正在根据评审意见修复...', progress: iterProgress });
    const repairDiagnostic = diagnoseHtmlCompleteness(currentHtml);
    const repairResult = await repairArtifact({
      html: currentHtml,
      ...
      plan: {
        ...
        gradeLevel: currentPlan.gradeLevel,
        learningGoals: currentPlan.learningGoals,
        variables: currentPlan.variables.map((v) => ({ ...v })),
        widgetOutline: currentPlan.widgetOutline as unknown as Record<string, unknown>,
      },
      blueprint,
      schemaValidation: toSchemaValidationSummary(validationResult),
      decision,
      issues: decision.blockingIssues,
      diagnostic: repairDiagnostic,
    });

    if (repairResult.revisedHtml) {
      currentHtml = repairResult.revisedHtml;
    }
    ...
    allChangeLog.push(...repairResult.changeLog);
    emit({ type: 'repair_completed', iteration, changeLog: repairResult.changeLog, progress: iterProgress });

    // Rebuild schema with updated content
    currentSchema = createSchemaWithWidget(currentPlan, currentHtml, widgetConfig, currentActions);
  ...
  const feedbackResult: FeedbackLoopResult = {
    passed: bestScore >= 85 && (lastDecision?.type === 'accept'),
    finalScore: bestScore,
    iterations,
    finalIssues: iterations.flatMap((i) => i.evaluations.flatMap((e) => e.issues)),
    bestVersionReason: iterations.length > 1
      ? `经过 ${iterations.length} 轮评审，最终评分 ${bestScore}/100`
      : '首轮评审即通过。',
  };
  ...
  const finalArtifact: InteractionArtifact = {
    ...bestArtifact,
    feedbackLoop: feedbackResult,
    qualityReport,
    finalScore: bestScore,
    generationIterations: iterations.length,
    changeLog: allChangeLog,
  };

  emit({ type: 'feedback_completed', result: feedbackResult, qualityReport, progress: 99 });
  ...
  log.info('Pipeline completed', { elapsed: `${totalElapsed}s`, artifactId: finalArtifact.id, artifactSize: `${(artifactSize / 1024).toFixed(1)}KB`, finalScore: bestScore, iterations: iterations.length });
}
...
async function buildWidgetHtml(plan: InteractionPlan, blueprint: LearningBlueprint): Promise<string> {
...
3. 控制区、主舞台、观察数据区、公式区和问题区必须有稳定 id 或 data-role，便于教师动作定位。`;
  ...
  try {
    ...
    const repaired = await repairWidgetHtml(plan, html, validationError, diagnostic, blueprint);
...
async function ensureWidgetHtmlSafety(
  ...
  try {
    ...
    try {
      ...
      const repaired = await repairWidgetHtml(plan, fixed, validationError, diagnostic, blueprint);
      const repairedFixed = repairMalformedHtml(repaired);
      assertSafeInteractiveHtml(repairedFixed);
      return repairedFixed;
    }
...
The widget must be fully functional with complete interactive JavaScript code.
...
Actions must help students observe core variables, satisfy the expectedInsight, and highlight important formulas, units, or stage order from the LearningBlueprint.
