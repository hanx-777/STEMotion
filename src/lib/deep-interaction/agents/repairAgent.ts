  ...
  const raw = await withTimeout(
    generateWithConfiguredModel({
      messages: [
        {
          role: 'system',
          content: `你是一个 HTML 修复专家。根据提供的问题列表修复 HTML 代码。
返回修复后的完整 HTML 文档。不要写 markdown。
保留原有的教学内容和交互逻辑，只修复指出的问题。
必须保留：widget-config、postMessage 监听器、SET_WIDGET_STATE/HIGHLIGHT_ELEMENT/ANNOTATE_ELEMENT/REVEAL_ELEMENT 消息处理。
必须保留：requestAnimationFrame 动画循环、开始/重置按钮。
安全要求：只允许内联 CSS/JS，禁止 fetch、WebSocket、XMLHttpRequest 等外部请求。
如果提供了 LearningBlueprint，修复后必须继续满足 expectedInsight、must 级知识约束和 coreVariables；不要删除核心变量控件或观察结果。
