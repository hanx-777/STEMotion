...
(function(){
  ...
  window.draw = window.draw || function(){
    ...
    if (stage) stage.setAttribute('data-running', widgetState.running ? 'true' : 'false');
  };
  window.update = window.update || function(){ window.draw(); };
  resetFromConfig();
  document.querySelectorAll('input[type="range"]').forEach(function(input){
    ...
    input.addEventListener('change', function(){ window.update(); });
  });
