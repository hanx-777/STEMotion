...
import { decideVisualization } from './visualization/decision_agent';
import { generateVisualizationSpec } from './visualization/spec_generator';
import { checkVisualizationSpec } from './visualization/quality_checker';
...
export async function askRag(input: RagAskInput, options: RagPipelineOptions = {}): Promise<RagAskResult> {
  ...
  // Visualization decision + spec generation
  const vizDecision = decideVisualization({ question, answerText: draft.answer });
  const vizSpec = vizDecision.shouldVisualize
    ? generateVisualizationSpec({ decision: vizDecision, question, answerText: draft.answer })
    : undefined;
  const validatedVizSpec = vizSpec && checkVisualizationSpec(vizSpec).passed ? vizSpec : undefined;
  ...
  return {
    ...
    visualization_hint: finalDraft.visualizationHint ?? draft.visualizationHint ?? visualizationHint,
    visualization_spec: validatedVizSpec,
    citations,
      ...
      messages: [
        {
          ...
          content: [
            ...
            'The answer must be grounded in the evidence pack. If evidence is insufficient, explicitly state that no reliable evidence was found, but still answer with general subject reasoning.',
          ].join('\n\n'),
        },
...
function buildUserPrompt(
  ...
  return [
    `Question: ${question}`,
    `Task type: ${taskTypeLabel(taskType)}`,
    `Answer template: ${answerTemplate}`,
    `Instruction: ${noEvidenceInstruction}`,
    'Use the following JSON schema and keep formulas in formula_blocks rather than inline code fences.',
    'CRITICAL: All math formulas MUST use LaTeX. Inline: $f\'(x)$ or \\(f\'(x)\\). Display: $$f(x)=xe^{-x^2}$$ or \\[f(x)=xe^{-x^2}\\]. Never use plain text like e^(-x^2), sqrt(x), 1/2, x^2.',
