...
export const useInteractionSessionStore = create<InteractionSessionState>()(
  ...
  createSession: (session) =>
    set((state) => {
      const sessions = normalizeSessions([session, ...state.sessions.filter((item) => item.id !== session.id)]);
      return {
        sessions,
        currentSessionId: resolveCurrentSessionId(sessions, session.id),
      };
    }),
  ...
  setCurrentSession: (sessionId) =>
    set((state) => ({
      currentSessionId: resolveCurrentSessionId(state.sessions, sessionId),
    })),
  ...
  addArtifact: (sessionId, artifact) =>
    set((state) => ({
      sessions: normalizeSessions(state.sessions.map((session) =>
        session.id === sessionId
          ? {
              ...session,
  ...
  deleteSession: (sessionId) =>
    set((state) => {
      const sessions = normalizeSessions(state.sessions.filter((session) => session.id !== sessionId));
      return {
        sessions,
        currentSessionId: state.currentSessionId === sessionId
          ? sessions[0]?.id ?? null
          : resolveCurrentSessionId(sessions, state.currentSessionId),
      };
    }),
  ...
  getCurrentSession: () => {
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        const state = persistedState as Partial<InteractionSessionState> | undefined;
        const sessions = normalizeSessions(Array.isArray(state?.sessions) ? state.sessions : []);
        return {
          sessions,
          currentSessionId: resolveCurrentSessionId(sessions, state?.currentSessionId ?? null),
        };
      },
      partialize: (state) => {
        ...
        return {
          sessions,
          currentSessionId: resolveCurrentSessionId(sessions, state.currentSessionId),
        };
      },
    },
  ),
