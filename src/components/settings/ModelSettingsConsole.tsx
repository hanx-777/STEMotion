...
export default function ModelSettingsConsole() {
  ...
  useEffect(() => {
    ...
    if (!list || loading || prefersReducedMotion()) return;

    const cards = Array.from(list.querySelectorAll('[data-profile-card]'));
  ...
  useEffect(() => {
    if (status?.type !== 'success' || !selectedId || prefersReducedMotion()) return;
    const card = Array.from(profileListRef.current?.querySelectorAll<HTMLElement>('[data-profile-card]') ?? [])
      .find((item) => item.dataset.profileId === selectedId);
    if (!card) return;

    gsap.fromTo(
      card,
      { scale: 0.99, boxShadow: '0 0 0 0 rgba(15, 118, 110, 0)' },
      {
        scale: 1,
      ...
      <div ref={settingsMotionRef} className="mx-auto grid w-full max-w-7xl gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <main data-settings-motion className="space-y-5">
          <section className="stemotion-elevated rounded-lg p-5">
            ...
            <div ref={formGridRef} className="mt-5 grid gap-4 lg:grid-cols-2">
              ...
              <label data-settings-field className="flex min-h-11 items-center gap-2 rounded-lg border border-[var(--stemotion-border)] bg-[#fbfaf6] px-3 text-sm font-semibold text-[var(--stemotion-ink)]">
                <input
                  type="checkbox"
...
function Field({ label, helper, children }: { label: string; helper?: string; children: React.ReactNode }) {
  return (
    <label data-settings-field className="block text-sm font-semibold text-[var(--stemotion-ink)]">
      {label}
