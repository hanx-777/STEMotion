...
export type VisualizationMode = 'auto' | 'manual' | 'off';
// 'auto': automatic generation based on keyword detection (default)
// 'manual': skip automatic generation, return hint only
// 'off': completely disable visualization processing
...
export function toLegacyRagResult(result: RagV1AskResponse): RagAskResult {
  return {
    ...
    should_generate_visualization: result.shouldGenerateVisualization,
    citations: result.citations,
