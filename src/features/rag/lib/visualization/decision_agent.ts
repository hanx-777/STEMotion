...
export function decideVisualization(input: {
  ...
  const scores: Array<{ type: VisualizationType; score: number; matched: string[]; minConfidence: number }> = [];
  ...
  for (const pattern of DETECTION_PATTERNS) {
    ...
    scores.push({ type: pattern.type, score, matched, minConfidence: pattern.minConfidence });
  }
  ...
  const passedThreshold = best.score >= best.minConfidence;
  return {
    shouldVisualize: passedThreshold,
    visualizationType: passedThreshold ? best.type : undefined,
    confidence: best.score,
    reason: passedThreshold
