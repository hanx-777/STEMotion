...
import { LLM_REQUEST_PRESETS } from '@/lib/generation/llmRequestBuilder';
...
export const HTML_GENERATION_MAX_TOKENS = LLM_REQUEST_PRESETS[HTML_GENERATION_REQUEST_PRESET].maxTokens;
...
export interface HtmlGenerationInput {
  ...
  /** Round 002B: Lightweight plan injected from the 5-agent pipeline. */
...
export async function generateInteractiveHtml(
  ...
  try {
    raw = await generate({
      ...
      temperature: 0.3,
      requestPreset: HTML_GENERATION_REQUEST_PRESET,
    });
  } catch (error) {
...
${buildInternalMultiAgentPlanningPrompt({
  mode: 'artifact',
  artifactKind: 'RAG problem-specific interactive visualization HTML',
  outputInstruction: '只返回完整 HTML，不要 Markdown、解释、JSON summary 或内部规划。',
})}

${input.lightweightPlan ? formatLightweightPlanForPrompt(input.lightweightPlan) : ''}
...
${buildRagVisualizationDesignContext({
  medium: 'RAG problem-specific interactive visualization HTML',
  originalQuestion: input.question,
  variables: input.brief?.variables ?? Object.entries(input.extractedParameters).map(([name, value]) => ({
    name,
    label: name,
    value: String(value),
  })),
  visualObjects: input.brief?.mustShow,
  metrics: input.finalResults?.map((item) => item.label),
  interactionIntent: input.brief?.visualGoal ?? input.visualizationType,
})}
...
10. 遵守共享 artifact design contract：
${artifactDesignContractPrompt({
  medium: 'RAG problem-specific interactive visualization HTML',
  mainStage: 'main visualization stage and result area',
  supportPanel: 'right explanation/sidebar areas',
  supportingContent: 'the original question, variables, demonstration plan, learning goals, citations, and long explanations',
})}

只返回HTML代码，不要解释。`;
}

export const HTML_GENERATION_SYSTEM_PROMPT = `你是 STEMotion RagInteractiveHtmlAgent。
...
${buildInternalMultiAgentPlanningPrompt({
  mode: 'artifact',
  artifactKind: 'RAG problem-specific interactive visualization HTML',
  outputInstruction: 'HTML only；不输出内部规划、Markdown 或解释。',
})}
${buildRagVisualizationDesignContext({
  medium: 'RAG problem-specific interactive visualization HTML',
  interactionIntent: 'create a compact first-screen learning interaction, not a generic explanation page',
})}
${artifactDesignContractPrompt({
  medium: 'RAG problem-specific interactive visualization HTML',
  mainStage: 'main visualization stage and result area',
  supportPanel: 'right explanation/sidebar areas',
  supportingContent: '辅助说明、变量、学习目标、演示计划、引用和长说明',
})}`;
