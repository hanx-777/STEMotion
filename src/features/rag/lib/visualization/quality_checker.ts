...
import { compileSafeFunctionExpression } from './safe_expression';
...
function checkFunctionGraph(
  ...
  for (const expr of spec.expressions) {
    ...
    if (!compiled.ok) {
      issues.push(`[ERROR] 表达式 ${expr.id} 的 evaluator 无法安全解析：${compiled.error}`);
    }
  }
...
function checkAlgorithmTrace(
  ...
  if (!spec.inputExample?.trim() || /示例输入|placeholder/i.test(spec.inputExample)) {
    issues.push('[ERROR] 算法过程缺少具体输入示例');
  }
  if (spec.steps.length < 2) {
    issues.push('[ERROR] 算法过程至少需要包含初始化和一次状态转移');
  }
  if (spec.steps.every((step) => /初始化|init/i.test(step.operation))) {
    issues.push('[ERROR] 算法过程不能只有初始化步骤');
  }
  ...
  for (let i = 0; i < spec.steps.length; i++) {
    ...
    if (!spec.steps[i].explanation?.trim()) {
      issues.push(`[WARNING] 步骤 ${spec.steps[i].stepIndex} 缺少说明`);
    }
  }
}
