...
export type RagPipelineEvent =
  ...
  | { type: 'answer_ready'; result: RagAskResult; elapsedMs?: number }
  | { type: 'quality_ready'; qualityReport: RagAskResult['quality_report']; elapsedMs?: number };
...
export async function askRag(input: RagAskInput, options: RagPipelineOptions = {}): Promise<RagAskResult> {
  ...
  const emit = (event: RagPipelineEvent) => options.onEvent?.({ ...event, elapsedMs: event.elapsedMs ?? Date.now() - startedAt });
  const checkAborted = () => {
  ...
  emit({ type: 'progress', stage: 'parse', message: '正在解析问题...', progress: 4 });
  checkAborted();
  ...
  emit({ type: 'progress', stage: 'retrieve_local', message: '正在检索本地课程资料...', progress: 14 });
  ...
  const webResults = shouldUseWeb
    ...
    : [];
  emit({ type: 'progress', stage: 'retrieve_web', message: shouldUseWeb ? '网络补充检索完成。' : '跳过网络补充检索。', progress: 24 });
  checkAborted();
  ...
  const draftResult: RagAskResult = {
    subject: subject.name,
    subject_display_name: subject.display_name,
    task_type: taskType,
    answer_protocol: draft.answerProtocol,
    answer: draft.answer,
    answer_sections: draft.answerSections,
    formula_blocks: draft.formulaBlocks,
    final_results: draft.finalResults,
    visualization_hint: draft.visualizationHint ?? visualizationHint,
    visualization_spec: visualizationMode === 'auto' ? validatedVizSpec : undefined,
    should_generate_visualization: visualizationMode === 'auto'
    ...
    quality_report: deterministicReport,
  };
  emit({ type: 'answer_ready', result: draftResult });
  ...
  const multiAgentResult = await runRagMultiAgentOrchestrator(
    ...
    },
    {
      ...
      reviewGenerator: options.reviewGenerator ?? ((llmOptions) => generateWithConfiguredModel({ ...llmOptions, signal: options.signal })),
      revisionGenerator: options.revisionGenerator ?? ((llmOptions) => generateWithConfiguredModel({ ...llmOptions, signal: options.signal })),
  ...
  checkAborted();
  emit({ type: 'quality_ready', qualityReport: multiAgentResult.qualityReport });
      ...
      messages: [
        {
          ...
          content: [
            ...
            buildInternalMultiAgentPlanningPrompt({
              ...
              outputInstruction: 'Use the existing JSON answer protocol only; do not output internal agent notes.',
            }),
