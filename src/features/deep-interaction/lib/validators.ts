...
export function validateInteractionArtifact(artifact: InteractionArtifact, options: InteractionValidationOptions = {}): void {
  assertText(artifact.id, 'artifact.id');
  assertText(artifact.sessionId, 'artifact.sessionId');
  assertAllowedType(artifact.type);
  validateInteractionSchema(artifact.schema, options);
}

export function validateInteractionSchema(schema: InteractionSchema, options: InteractionValidationOptions = {}): void {
