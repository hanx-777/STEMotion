...
import type { RagWidgetContractDiagnostic } from '@/features/rag/lib/visualization/widgetContract';
import { formatRagWidgetContractForPrompt } from '@/features/rag/lib/visualization/widgetContract';
import type { ActiveInteractionDiagnostics } from '@/features/rag/lib/visualization/activeInteractionDiagnostics';
import { formatActiveInteractionDiagnosticsForPrompt } from '@/features/rag/lib/visualization/activeInteractionDiagnostics';
...
export interface RepairContext {
  ...
  activeInteractionDiagnostic?: ActiveInteractionDiagnostics;
  signal?: AbortSignal;
}
...
async function repairHtml(ctx: RepairContext): Promise<string | null> {
  if (isActiveInteractionOnlyRepair(ctx)) {
    return repairActiveInteractionHtml(ctx);
  }
  ...
  const contractDiagnosticInfo = ctx.contractDiagnostic
    ...
    : '';
  const activeInteractionInfo = ctx.activeInteractionDiagnostic
    ? `\n\n${formatActiveInteractionDiagnosticsForPrompt(ctx.activeInteractionDiagnostic)}`
    : '';
...
6. 所有交互控件必须可操作，页面必须是完整可用的`
    : '';
  const activeInteractionInstructions = ctx.activeInteractionDiagnostic && !ctx.activeInteractionDiagnostic.passed
    ? `\n\n重要：主动浏览器验收失败。你必须补齐真实状态机和可见反馈：
1. start/reset/slider 的事件处理必须修改 state 中的题目变量或步骤状态
2. 每次操作后必须让 #visualization 或 #metrics 可见变化（SVG/Canvas/DOM 文本、transform、class、属性或像素）
3. 禁止只 postMessage ACK、alert()、console.log()、hover 效果或空事件监听
4. 禁止用静态 fallback 骨架冒充题目专属互动`
    : '';
...
- start/reset/slider 操作后的 #visualization 或 #metrics 可见变化。
...
${buildInternalMultiAgentPlanningPrompt({
  mode: 'repair',
  artifactKind: 'self-contained interactive widget HTML repair',
  outputInstruction: '只返回完整替换 HTML 文档。',
})}
${designRepairPrompt()}
${interactivityInstructions}
${activeInteractionInstructions}`,
        },
        {
          role: 'user',
          content: `## 需要修复的问题
...
async function repairActiveInteractionHtml(ctx: RepairContext): Promise<string | null> {
  const issuesDescription = ctx.issues
    .filter((i) => i.target === 'html' || i.target === undefined)
    .map((i) => {
      const evidence = i.evidence ? `；证据：${i.evidence}` : '';
      return `- [${i.severity}] ${i.message}${evidence}（建议：${i.suggestion}）`;
    })
    .join('\n');
  const activeInteractionInfo = ctx.activeInteractionDiagnostic
    ? formatActiveInteractionDiagnosticsForPrompt(ctx.activeInteractionDiagnostic)
...
任务：只修复主动交互失败，返回完整替换 HTML。

输出规则：
- 只返回完整 HTML 文档，不要写 Markdown、说明文字或局部 patch。
- 保留原题教学内容、变量、公式、widget-config 和 postMessage 协议。
- 保留开始、重置、滑块等控件；不要替换成静态说明页。

修复重点：
- start/reset/slider 必须修改真实 state。
- 每次操作后 #visualization 或 #metrics 必须发生可见变化。
- 可见变化可以是 SVG/Canvas/DOM 文本、transform、class、属性或像素变化。
- 禁止只 postMessage ACK、alert()、console.log()、hover 效果或空事件监听。
...
${buildInternalMultiAgentPlanningPrompt({
  mode: 'repair',
  artifactKind: 'targeted active-interaction HTML repair',
  outputInstruction: '只返回完整替换 HTML 文档。',
  compact: true,
})}`,
        },
        {
          role: 'user',
          content: `## 主动交互失败问题
${issuesDescription || 'start/reset/slider 操作后没有产生可见反馈。'}

## 修复指令
${ctx.decision.repairInstruction ?? '补齐 start/reset/slider 的真实状态机和可见反馈。'}
...
${ctx.html}`,
      ...
      signal,
      metadata: {
        stage: 'html_repair_active_interaction',
        promptVersion: 'html-repair-active-interaction-v1',
      },
    }),
    120000,
    'Repair timed out.',
...
function isActiveInteractionOnlyRepair(ctx: RepairContext): boolean {
  if (!ctx.activeInteractionDiagnostic || ctx.activeInteractionDiagnostic.passed) return false;
  if (ctx.diagnostic || ctx.contractDiagnostic) return false;
  if (ctx.schemaValidation && (!ctx.schemaValidation.passed || ctx.schemaValidation.violations.length > 0)) return false;

  const htmlIssues = ctx.issues.filter((issue) => issue.target === 'html' || issue.target === undefined);
  if (htmlIssues.length === 0) return true;

  return htmlIssues.every((issue) => {
    const text = `${issue.id}\n${issue.message}\n${issue.evidence ?? ''}\n${issue.suggestion}`;
    return issue.category === 'runtime'
      && /active[_\s-]?interaction|主动交互|start|reset|slider|滑块|可见变化/i.test(text);
  });
}
...
${buildInternalMultiAgentPlanningPrompt({
  mode: 'repair',
  artifactKind: 'teacher action JSON repair',
  outputInstruction: '返回纯 JSON：{"actions":[...]}，不要 Markdown 或内部规划。',
  compact: true,
})}`,
        },
        {
...
${JSON.stringify(ctx.actions, null, 2).slice(0, 3000)}`,
      ...
      signal,
      metadata: {
        stage: 'teacher_action_repair',
        promptVersion: 'teacher-action-repair-v1',
      },
    }),
    60000,
    'Repair timed out.',
