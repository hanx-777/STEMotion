...
test('generates projectile_motion spec from decision', () => {
  ...
  const spec = generateVisualizationSpec({ decision, question: '斜抛问题' });
  assert.ok(spec);
...
test('returns undefined when shouldVisualize is false', () => {
  const decision: VisualizationDecision = {
  ...
  const spec = generateVisualizationSpec({ decision, question: '导数定义' });
  assert.equal(spec, undefined);
});

test('generates function_graph spec for f(x) expression', () => {
  const decision: VisualizationDecision = {
    shouldVisualize: true,
...
test('generates force_diagram spec for incline question', () => {
  const decision: VisualizationDecision = {
    shouldVisualize: true,
...
test('generates algorithm_trace spec for stack question', () => {
  const decision: VisualizationDecision = {
    shouldVisualize: true,
