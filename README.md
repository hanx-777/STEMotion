...
![STEMotion 学生学习主入口](docs/assets/readme/learn-page-live.jpg)
...
![模型与 API 设置页](docs/assets/readme/settings-page-live.jpg)
...
4. 检查回答中的本地来源 `[Lx]`、证据摘要和可视化提示。
5. 跳转 `/lab`，把同一知识点转成可交互实验。

## 系统架构摘要

系统由 Next.js 前端页面、Route Handlers、Subject Skill 配置、本地 RAG 检索、可选网络补充检索和 OpenAI / Claude 模型调用组成。`/learn` 与 `/teach` 页面负责教学任务交互和轻量可视化展示；`/lab` 继承 deep-interaction 完整交互实验生成；`/knowledge` 展示知识库健康度；`/assets` 管理教学资产；`/api/v1/rag/ask` 负责读取当前学科 Skill、检索本地课程资料、合并网络补充资料，并让当前启用的大模型生成最终回答。旧版 `/api/rag/ask` 仍作为兼容 adapter 保留。
