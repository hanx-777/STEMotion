...
- 参赛展示页：`/student`（`/rag` 作为旧链接兼容入口，会重定向到 `/student`）
...
`/student` 是 XH202620 参赛展示主页面，面向评委展示“学科垂类 RAG + 可追溯引用 + 教学任务 + 运动可视化”闭环。`/rag` 保留为旧链接兼容入口。
...
系统由 Next.js 前端页面、Route Handlers、Subject Skill 配置、本地 RAG 检索、可选网络补充检索和 OpenAI / Claude 模型调用组成。`/student` 与 `/teacher` 页面负责教学任务交互和轻量可视化展示；`/visualization` 继承 deep-interaction 完整交互实验生成；`/api/v1/rag/ask` 负责读取当前学科 Skill、检索本地课程资料、合并网络补充资料，并让当前启用的大模型生成最终回答。旧版 `/api/rag/ask` 仍作为兼容 adapter 保留。
...
RAG 工作台支持四类教学任务，其中 `/student` 暴露知识问答、分步解题和错因诊断，`/teacher` 暴露教师备课相关任务：
...
2. `/student` 默认学科为大学物理力学，`/rag` 会重定向到 `/student`。
